...
CREATE TABLE Internação(
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_medi_idx` (`id_medico` ASC) VISIBLE,
  INDEX `fk_paci_idx` (`id_paciente` ASC) VISIBLE,
  INDEX `fk_quar_idx` (`id_quarto` ASC) VISIBLE,
  INDEX `fk_enfe_idx` (`id_enfermeiros` ASC) VISIBLE,
  CONSTRAINT `fk_medi`
  ...
  CONSTRAINT `fk_enfe`
    FOREIGN KEY (`id_enfermeiros`)
    REFERENCES `hospital_db_Diogo`.`Enfermeiro_internação` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
